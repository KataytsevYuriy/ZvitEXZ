    ...
    internal class ListShurvesTiArray
    {
        public string[,] Convert(List<Shurf> shurves)
        {
            ...
            foreach (Shurf shurf in shurves)
            {
                ...
                res[i, 3] = shurf.ShurfLength;
                res[i, 4] = shurf.IsolationSquare;
                res[i, 5] = shurf.IsolationGettingSquare;
                ...
                res[i, 22] = shurf.Thin1SharUp;
                res[i, 23] = shurf.Thin1SharDown;
                res[i, 24] = shurf.Thin1SharRight;
                res[i, 25] = shurf.Thin1SharLeft;
                res[i, 26] = shurf.Thin2SharUp;
                res[i, 27] = shurf.Thin2SharDown;
                res[i, 28] = shurf.Thin2SharRight;
                res[i, 29] = shurf.Thin2SharLeft;
                res[i, 30] = shurf.IsHruntovka;
                res[i, 31] = shurf.Adheziya;
                res[i, 32] = shurf.Vologa;
                res[i, 33] = shurf.PipeTemperature;
                res[i, 34] = shurf.StanPoverhnyPipe;
                res[i, 35] = shurf.SquareMetalPipe;
                res[i, 36] = shurf.IsKorozyaExist;
                res[i, 37] = shurf.CharacterKorozii;
                res[i, 38] = shurf.KavernLength;
                res[i, 39] = shurf.KavernHeight;
                res[i, 40] = shurf.KavernPosition;
                res[i, 41] = shurf.UtzBeforShurf;
                res[i, 42] = shurf.UtzInShurf;
                res[i, 43] = shurf.UtzAfterShurf;
